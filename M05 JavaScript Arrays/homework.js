...
function promedioResultadosTest(resultadosTest) {
    ...
    // Tu código:
    var total = 0
    var n = 1.0 * resultadosTest.length
    //  for(var i=0; i<resultadosTest.length;i++){
    //    total = total + resultadosTest[i]
    //  }
    return (
        resultadosTest.reduce((suma, valor) => {
            return suma + valor
        }, 0) / resultadosTest.length
    )
...
function multiplicarArgumentos() {
    ...
    return argumentos.length > 0
        ? argumentos.reduce((acumulador, valor) => {
...
function mesesDelAño(array) {
    ...
    // Tu código:
    var arregloPedido = array.filter((element) => {
        return (
            element === 'Enero' ||
            element === 'Marzo' ||
            element === 'Noviembre'
...
function breakStatement(num) {
    ...
    return coinciden ? 'Se interrumpió la ejecución' : arr
}
